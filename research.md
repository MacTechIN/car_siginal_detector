...
## 10. 학습 데이터 수집 방안 (한국 신호등)

| 방안 | 장점 | 단점 | 상태 |
|---|---|---|---|
| **A. 운전 중 음성 라벨링** (본 프로젝트) | 내 카메라와 내 도로의 데이터라 도메인 차이가 없음. 손을 쓰지 않아 안전. 라벨 시점이 정확함 | 수집 속도가 주행량에 비례 | ✅ 구현 (`--label-voice`) |
| B. 녹화본을 재생하며 음성 라벨링 | 운전 부담 없음. 같은 구간을 반복해서 라벨링 가능 | 추가 시간 필요 | ✅ 가능 (`--source <녹화> --realtime --label-voice`) |
| C. AI Hub 신호등 데이터 | 대량 (신호등 박스 약 300만 개) | 내국인만 신청 가능, R&D 용도, 재배포 금지, 카메라가 달라 도메인 차이 | 신청 필요 → `data/extra_tl`로 투입 |
| D. "맞아" 확인 방식의 반자동 라벨 | 규칙이나 모델이 맞힌 것을 한 단어로 확정 | 틀린 판별을 확정할 위험 | ✅ 구현 |
| E. 자동 라벨(고신뢰 예측) + 사람 검토 | 확장성 | 검토 도구가 없음 | 다음 단계 |

### 음성 인식 선택: Vosk (오프라인)

- 모델: `vosk-model-small-ko-0.22` (87MB, alphacephei.com, Apache-2.0). CPU에서 실시간 동작하고 인터넷 없이 됩니다.
- **자유 인식은 부적합했습니다.** 합성 음성 "빨간불"이 "발덴 본다", "좌회전"이 "자위 든"으로 나왔습니다.
- **문장 목록(grammar) 모드**는 라벨 19개 중 17개를 맞혔습니다. 어휘에 없는 "빨강", "꺼짐"은 제외했습니다.
- 문제: 이 모델은 `[unk]`(모르는 말)을 지원하지 않습니다. 그래서 문장 목록 모드에서는 **일반 대화도 라벨 문장으로 강제 인식되고, 신뢰도가 1.0까지 나옵니다.**
  - 예: "오늘 날씨가 정말 좋네요" → "노란 적색 점멸 점멸", "아 배고프다" → "황색 맞아"
- 해결:
  1. 일상 단어 약 150개를 방해 단어로 추가했습니다.
  2. 발화 전체가 라벨 문장과 정확히 같을 때만 받아들입니다.
  3. 소리가 비슷한 한 음절 방해 단어(차, 비, 셋, 넷 등)는 뺐습니다. 라벨 뒤에 붙어 인식을 망가뜨렸기 때문입니다.
- 결과 (`tools/eval_voice_labels.py`, Heami 합성 음성): **라벨 21/21 정확, 일반 대화 15문장 중 오인 0.**
- 한계: 실제 사람 목소리, 차량 소음, 라디오 환경에서는 미검증입니다. 시연 녹화의 `labels.csv`로 확인합니다.
- 대안 후보(미적용): faster-whisper(정확하지만 CPU 지연이 큼), Windows 음성 인식(한국어 인식기 설치 여부 불확실).
